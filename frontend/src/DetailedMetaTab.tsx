        ...
        render: () => {
            return (
                <MaterialTable
                    ...
                    data={Object.entries(rowData.data!)
...
function CategorizedMeta({ meta }: MetaViewProps) {
    ...
    return (
        <Grid container>
            <Grid item xs={2}>
                <Tabs orientation="vertical" variant="scrollable" value={tabValue} onChange={handleTabChange} aria-label="Vertical tabs example" className={classes.tabs}>
                    {isMainCategory && <Tab classes={{ wrapper: classes.tabLabel }} label="general" {...a11yProps(0)} />}
        ...
        <div>
            <Grid container alignItems="baseline">
                ...
                <FormControlLabel
                    ...
                    control={<Switch checked={plainView} name="checked" onChange={handleChange} color="primary" size="small" />}
                    label="Plain View"
                />
            </Grid>
            <Paper className={classes.paper}>{plainView ? <PlainMeta meta={meta} /> : <CategorizedMeta meta={meta} />}</Paper>
        </div>
    );
}
...
function DetailedMetaTab({ meta, tableFromMeta, isBeamtime }: StaticMetaProps) {
    return (
        <div>
            ...
            {meta.customValues && <CustomMeta meta={meta} />}
        </div>
    );
}
