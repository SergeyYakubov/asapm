import { useQuery, gql, InMemoryCache, makeVar } from '@apollo/client';
...
function TableDataFromCollections(collections: CollectionEntry[], columns: ColumnList): BasicCollectionDetails[] {
    return collections.map((collection) => {
        const res: BasicCollectionDetails = {
            id: collection.id,
...
function possibleColumnListfromCustomValues(vals: KvObj | null, root: string, columns: ColumnList) {
    ...
    for (const [column, value] of Object.entries(vals)) {
        ...
        } else if (!columns.find((c) => c.fieldName === `customValues.${fullColumn}`)) {
            columns.push({
...
function PossibleColumnListfromCollections(currentColumns: ColumnList, collections: CollectionEntry[]) {
    const columns = currentColumns.map((a) => ({ ...a }));
...
function SelectColumns({ collections, columns, close }: SelectColumnsProps) {
    ...
    const UpdateAlias = (newValue: any, oldValue: any, rowData: ColumnItem, columnDef: Column<any>): Promise<void> => {
    ...
    const handleSelectionChange = (data: ColumnList) => {
...
const defaultColumns: ColumnList = [
    ...
    {
        fieldName: 'parentBeamtimeMeta.id',
        alias: 'Beamtime ID',
        active: true,
        type: null,
    },
    {
        fieldName: 'parentBeamtimeMeta.beamline',
        alias: 'Beamline',
        active: true,
        type: null,
    },
    {
        fieldName: 'parentBeamtimeMeta.facility',
        alias: 'Facility',
        active: true,
        type: null,
    },
    {
        fieldName: 'parentBeamtimeMeta.users.doorDb',
        alias: 'Door users',
        active: true,
        type: null,
    },
...
export const columnsVar = makeVar<ColumnList>(defaultColumns);
...
function CollectionTable({ collections }: CollectionProps) {
    ...
    const handleClick = (event?: React.MouseEvent, rowData?: BasicCollectionDetails, toggleDetailPanel?: (panelIndex?: number) => void) => {
    ...
    return (
        <div>
            <Box display="flex" justifyContent="flex-end" className={classes.toolBox}>
                <IconButton color="secondary" aria-label="select columns" aria-haspopup="true" onClick={handleColumnButtonClick}>
                    <ViewColumnIcon />
                </IconButton>
                <Popover
                    id="simple-menu"
            ...
            <MaterialTable
                ...
                columns={columns
                    ...
                    .map((column) => {
                        return {
                            title: column.alias || column.fieldName,
