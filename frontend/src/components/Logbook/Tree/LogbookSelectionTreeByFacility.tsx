...
export function TreeBodyByFacility({
                                messages,
                                currentVisibleDate,
                                onDateSelected
                            }: { messages: LogEntryMessage[], currentVisibleDate: string, onDateSelected: (fullDate: string) => void }): JSX.Element {
            ...
            Object.keys(preGroupByFacility).sort((a, b) => a.localeCompare(b)).map((facility) =>
                <TreeItem key={`tree,facility:${facility}`} nodeId={`facility:${facility}`} label={facility}>
                    {Object.keys(preGroupByFacility[facility]).sort((a, b) => Number(b) - Number(a)).map(year =>
                        <TreeItem key={`tree,year:${year},facility:${facility}`} nodeId={`year:${year},${facility}`}
                                  label={year}>
                            {Object.keys(preGroupByFacility[facility][year]).slice().reverse().map(month =>
                                <TreeItem key={`tree,month:${month},year:${year},facility:${facility}`}
                                          nodeId={`month:${month},${year},${facility}`}
                                          label={monthIdxToText(Number(month))}>
                                    {preGroupByFacility[facility][year][Number(month)].map(date =>
                                        <TreeItem key={`tree,facility:${facility},date:${date}`}
                                                  nodeId={`date:${date},${facility}`} label={date}/>
                                    )}
                                </TreeItem>
                            )}
                        </TreeItem>
