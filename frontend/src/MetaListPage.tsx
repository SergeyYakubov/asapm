...
function MetaColumn({ queryResult, status, title }: MetaColumnProps) {
    ...
    if (queryResult.loading || queryResult.error) {
        return (
            <Paper className={clsx(classes.paper, classes.paperNoReducedPadding)}>
                <div>{queryResult.loading ? <CircularProgress /> : <p>{queryResult.error!.message}...</p>}</div>
            </Paper>
        );
    }
            ...
            <Grid item xs={12}>
                <Paper className={classes.paper}>
                    <List component="nav">
                        {queryResult.data &&
                            queryResult
                                .data!.meta.filter((meta) => meta.status === status)
                                .map((meta) => (
                                    <ListItem button className={classes.listItem} onClick={handleClick} id={meta.id as string} key={meta.id as string}>
                                        <ListItemText
                                            ...
                                            secondary={
                                                <Grid container justify="space-between" component="span">
                                                    <>
                                                        <Typography component="span">Beamtime ID: {meta.id}</Typography>
