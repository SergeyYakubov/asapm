    ...
    return (
        <Breadcrumbs aria-label="breadcrumb">
            ...
            {cols.map((value) => {
                curPath += `.${value}`;
                return (
                    <Link color="inherit" to={`${curPath}/meta`}>
                        {value}
                    </Link>
                );
            })}
            <Link color="textPrimary" to={path} aria-current="page">
                {last}
            </Link>
        </Breadcrumbs>
    );
    ...
    return (
        <div className={classes.header}>
            ...
            <Grid container direction="row" justify={isBeamtime ? 'space-between' : 'flex-end'} alignItems="flex-end">
                {isBeamtime && (
                    <Chip
                        label={(meta as BeamtimeMeta).status}
                        variant="outlined"
                ...
                <FormControlLabel control={<Switch checked={rawView} onChange={handleChange} name="checked" color="primary" size="small" />} label="Raw JSON" />
            </Grid>
        </div>
    );
}
    ...
    if (typeof queryResult === 'string') {
        return (
            <div className={classes.root}>
                ...
                <Typography variant="h3">{queryResult as string}</Typography>
            </div>
        );
    }

    const data: BeamtimeMeta | CollectionEntry = isBeamtime ? queryResult.data!.meta[0] : queryResult.data!.collections[0];
    return (
        <div className={classes.root}>
            <Toolbar variant="dense" />
            <DetailedHeader meta={data} rawView={rawView} setRawView={setRawView} isBeamtime={isBeamtime} />
            {rawView ? <RawMeta meta={data} /> : <DetailedTabs meta={data} section={section} isBeamtime={isBeamtime} />}
        </div>
    );
}
